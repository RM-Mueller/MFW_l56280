---
title: "Measuring the Impact of Data Anomalies on Tourism Demand Forecasts"
...
library("ggpubr") # library containing Quantile-Quantile Plot

# Change locale in R to get the month name in English
Sys.setlocale("LC_TIME", "English")

# Hide the dplyr messages for summarise function 
...
```{r}
# Ratio of Portuguese Tourists in Aug 2019
residence_2019 %>%
  filter(Residence_Country == "Portugal") %>%
  arrange(desc(Ratio)) %>%
  select(-Residence_Country) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Ratio of Portuguese Tourists in Aug 2019</span>", digits = 4, format.args = list(big.mark = ","), align = "lcc") %>%
  column_spec(2:3, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = F)
```
...
```{r}
# Ratio of Portuguese Tourists in Aug 2021
residence_2021 %>%
  filter(!Region == ("Madeira"), !Region == ("Azores")) %>%
  filter(Residence_Country == "Portugal") %>%
  arrange(desc(Ratio)) %>%
  select(-Residence_Country) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Ratio of Portuguese Tourists in Aug 2021</span>", digits = 4, format.args = list(big.mark = ","), align = "lcc") %>%
  column_spec(2:3, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = F)
```

## Correlation: Ratio of Port. Tourists & Decline in Tourism

**Ratio of Overnight Stays by Portuguese Tourists in August 2019 & Mean year-over-year relative Change of Overnight Stays from April to December 2020**

```{r}
# Mean year-over-year relative Change: April - December 2020
seasdiff_apr20todec20 <- tourism %>%
  filter(Region != "Total Portugal") %>%
  mutate(SeasDiff_abs = difference(Overnight_Stays,12)) %>%
  filter_index("Apr 2020" ~ "Dec 2020") %>%
  mutate(SeasDiff_rel = SeasDiff_abs/(Overnight_Stays-SeasDiff_abs)) %>%
  features(SeasDiff_rel, features = list(mean = mean)) %>%
  rename(Mean_SeasDiff_rel_Apr20toDec20 = mean) %>%
  arrange(Mean_SeasDiff_rel_Apr20toDec20)

# Ratio of Portuguese Tourists in Aug 2019
residence_ratio_2019 <- residence_2019 %>%
  filter(Residence_Country == "Portugal") %>%
  arrange(desc(Ratio)) %>%
  select(c(Region, Ratio))

# Merge both tables
seasdiff_residence <- merge(residence_ratio_2019, seasdiff_apr20todec20, by = "Region") %>% 
  arrange(Ratio) %>%
  rename(Ratio_Port_Tourists_Aug19 = Ratio)
  
seasdiff_residence %>% 
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Ratio of Portuguese Tourists & Decline in Tourism due to Covid-19</span>", digits = 4, format.args = list(big.mark = ","), align = "lcc") %>%
  column_spec(2:3, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), full_width = F)
...
seasdiff_residence %>% 
  ggplot() +
  aes(x = Ratio_Port_Tourists_Aug19, y = Mean_SeasDiff_rel_Apr20toDec20) +
  geom_point(colour = "#0c4c8a") +
  geom_smooth(formula = y ~ x, method = lm, se = FALSE) + # fits a linear model and adds a line to plot
  ggtitle("Scatter Plot", subtitle = "Ratio of Portuguese Tourists & Decline in Tourism due to Covid-19") +
  xlab("Ratio of Overnight Stays by Portuguese Tourists in Aug 2019") +
  ylab("Mean year-over-year rel. Change in Overnight Stays Apr-Dec 2020")

# Pearson correlation coefficient between the two variables
cor(seasdiff_residence$Ratio_Port_Tourists_Aug19, seasdiff_residence$Mean_SeasDiff_rel_Apr20toDec20) %>% 
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Correlation Coefficient</span>") %>% 
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), full_width = F)
```
The scatter plot shows that there are no outliers in the data and that there is a linear relationship between the two variables. Furthermore, the Pearson correlation coefficient indicates a strong positive correlation, which means that if the proportion of Overnight Stays by Portuguese tourists in August 2019 is higher, the mean year-over-year relative Change of Overnight Stays from April to December 2020 is higher.
However, this does not mean that the correlation in the population is significantly different from 0. For this, a correlation test is needed.
To use the Pearson's correlation test, both variables have to be normally distributed. Therefore, we will now test for normal distribution:

```{r}
# Shapiro-Wilk normality test
shapiro.test(seasdiff_residence$Ratio_Port_Tourists_Aug19)
shapiro.test(seasdiff_residence$Mean_SeasDiff_rel_Apr20toDec20)

# Draw Quantile-Quantile (Q-Q) Plots
ggqqplot(seasdiff_residence$Ratio_Port_Tourists_Aug19) +
  ggtitle("Quantile-Quantile Plot", subtitle = "Ratio of Overnight Stays by Portuguese Tourists in Aug 2019")

ggqqplot(seasdiff_residence$Mean_SeasDiff_rel_Apr20toDec20) +
  ggtitle("Quantile-Quantile Plot", subtitle = "Mean year-over-year rel. Change in Overnight Stays Apr-Dec 2020")
```
The p-values of the Shapiro-Wilk normality tests are higher than 0.05 for both variables, which means that we do not reject the null hypothesis that the data follows a normal distribution. The Quantile-Quantile Plots seem to confirm this observation as all points are close to the reference line. There is only one observation that is slightly outside of the 95% confidence interval (grey area), but as the sample size is small, this is acceptable. Let's run the Pearson's correlation test:

```{r}
# Pearson Correlation Test
cor.test(seasdiff_residence$Ratio_Port_Tourists_Aug19, seasdiff_residence$Mean_SeasDiff_rel_Apr20toDec20)
```
The p-value of the Pearson's correlation test is 0.0005. At the 5% significance level, we do reject the null hypothesis that there is no correlation  between these two variables. In other words, we conclude at the 5% significance level that there is a linear relationship between the two variables in the population.

**Further Measures for the Variables**

*Ratio of Overnight Stays by Portuguese Tourists in 2018 & 2019*
```{r}
# Import data
residence_total2018 <- read_excel("C:\\Users\\Rosan\\OneDrive - ISEG\\# Masterthesis\\My MFW\\MonthlyOvernightStays_withResidenceCountry_perRegion.xlsx",sheet = "2018_AllRegions") %>%
  select(c(Region, Residence_Country, Total))

residence_total2019 <- read_excel("C:\\Users\\Rosan\\OneDrive - ISEG\\# Masterthesis\\My MFW\\MonthlyOvernightStays_withResidenceCountry_perRegion.xlsx",sheet = "2019_AllRegions") %>%
  select(c(Region, Residence_Country, Total)) 

# Merge both tables
residence_total2018_2019 <- add_row(residence_total2018, residence_total2019) %>%
  arrange(Region) %>% 
  group_by(Region, Residence_Country) %>% 
  summarise(Total_18_19 = sum(Total)) %>% 
  mutate(Ratio_Port_Tourists_Total_18_19 = Total_18_19/sum(Total_18_19)) %>% 
  filter(Residence_Country == "Portugal") %>% 
  select(c(Region, Ratio_Port_Tourists_Total_18_19))

# Change Region Names to English
residence_total2018_2019["Region"][residence_total2018_2019["Region"] == "Açores"] <- "Azores"
residence_total2018_2019["Region"][residence_total2018_2019["Region"] == "A.M. Lisboa"] <- "Lisbon M.A."
residence_total2018_2019["Region"][residence_total2018_2019["Region"] == "Centro"] <- "Central"
residence_total2018_2019["Region"][residence_total2018_2019["Region"] == "Norte"] <- "North"

# Add column to Correlation table
seasdiff_residence <- merge(seasdiff_residence, residence_total2018_2019, by = "Region")

# Check if variable is normally distributed
shapiro.test(seasdiff_residence$Ratio_Port_Tourists_Total_18_19)
```

*Ratio of Overnight Stays by Portuguese Tourists in Summer (Jun, Jul, Aug, Sep) in 2018 and 2019*
```{r}
# Import data
residence_summer2018 <- read_excel("C:\\Users\\Rosan\\OneDrive - ISEG\\# Masterthesis\\My MFW\\MonthlyOvernightStays_withResidenceCountry_perRegion.xlsx",sheet = "2018_AllRegions") %>%
  select(c(Region, Residence_Country, June, July, August, September)) %>% 
  group_by(Region) %>%
  mutate(Overnight_Stays_Summer = June + July + August + September) %>% 
  mutate(Ratio_18 = (Overnight_Stays_Summer)/sum(Overnight_Stays_Summer)) %>% 
  filter(Residence_Country == "Portugal") %>% 
  select(Region, Ratio_18)

residence_summer2019 <- read_excel("C:\\Users\\Rosan\\OneDrive - ISEG\\# Masterthesis\\My MFW\\MonthlyOvernightStays_withResidenceCountry_perRegion.xlsx",sheet = "2019_AllRegions") %>%
  select(c(Region, Residence_Country, June, July, August, September)) %>% 
  group_by(Region) %>%
  mutate(Overnight_Stays_Summer = June + July + August + September) %>% 
  mutate(Ratio_19 = (Overnight_Stays_Summer)/sum(Overnight_Stays_Summer)) %>% 
  filter(Residence_Country == "Portugal") %>% 
  select(Region, Ratio_19)

# Merge both tables
residence_summer2018_2019 <- merge(residence_summer2018, residence_summer2019, by = "Region") %>% 
    mutate(Ratio_Port_Tourists_Summer_18_19 = (Ratio_18 + Ratio_19)/2) %>% 
    select(c(Region, Ratio_Port_Tourists_Summer_18_19))

# Change Region Names to English
residence_summer2018_2019["Region"][residence_summer2018_2019["Region"] == "Açores"] <- "Azores"
residence_summer2018_2019["Region"][residence_summer2018_2019["Region"] == "A.M. Lisboa"] <- "Lisbon M.A."
residence_summer2018_2019["Region"][residence_summer2018_2019["Region"] == "Centro"] <- "Central"
residence_summer2018_2019["Region"][residence_summer2018_2019["Region"] == "Norte"] <- "North"

# Add column to correlation table
seasdiff_residence <- merge(seasdiff_residence, residence_summer2018_2019, by = "Region")

# Check if variable is normally distributed
shapiro.test(seasdiff_residence$Ratio_Port_Tourists_Summer_18_19)
```

*Mean year-over-year relative Change of Overnight Stays from March 2020 to February 2021*
```{r}
# Calculate Seasonal Difference from Mar 20 to Feb 21
seasdiff_mar20tofeb21 <- tourism %>%
  filter(Region != "Total Portugal") %>%
  mutate(SeasDiff_abs = difference(Overnight_Stays,12)) %>%
  filter_index("Mar 2020" ~ "Feb 2021") %>%
  mutate(SeasDiff_rel = SeasDiff_abs/(Overnight_Stays-SeasDiff_abs)) %>%
  features(SeasDiff_rel, features = list(mean = mean)) %>%
  rename(Mean_SeasDiff_rel_Mar20toFeb21 = mean)

# Add column to correlation table
seasdiff_residence <- merge(seasdiff_residence, seasdiff_mar20tofeb21, by = "Region")

# Check if variable is normally distributed
shapiro.test(seasdiff_residence$Mean_SeasDiff_rel_Mar20toFeb21)
```
*Mean year-over-year relative Change of Overnight Stays in Summer (Jun, Jul, Aug, Sep) in 2020 & 2021 to pre-Covid-19*
```{r}
# Calculate Seasonal Difference from Jun 20 to Sep 20
seasdiff_jun20tosep20 <- tourism %>%
  filter(Region != "Total Portugal") %>%
  mutate(SeasDiff_abs = difference(Overnight_Stays, 12)) %>%
  filter_index("Jun 2020" ~ "Sep 2020") %>%
  mutate(SeasDiff_rel = SeasDiff_abs/(Overnight_Stays-SeasDiff_abs)) %>%
  features(SeasDiff_rel, features = list(mean = mean)) %>%
  rename(Mean_SeasDiff_rel_Jun20toSep20 = mean)

# Calculate 2-year Seasonal Difference from Jun 21 to Sep 21
seasdiff_jun21tosep21 <- tourism %>%
  filter(Region != "Total Portugal") %>%
  mutate(SeasDiff_abs = difference(Overnight_Stays, 24)) %>%
  filter_index("Jun 2021" ~ "Sep 2021") %>%
  mutate(SeasDiff_rel = SeasDiff_abs/(Overnight_Stays-SeasDiff_abs)) %>%
  features(SeasDiff_rel, features = list(mean = mean)) %>%
  rename(Mean_SeasDiff_rel_Jun21toSep21 = mean)

# Merge both tables and calculate mean of Seasonal Differences
seasdiff_juntosep_20_21 <- merge(seasdiff_jun20tosep20, seasdiff_jun21tosep21, by = "Region") %>% 
  mutate(Mean_SeasDiff_rel_JuntoSep_20_21 = (Mean_SeasDiff_rel_Jun20toSep20 + Mean_SeasDiff_rel_Jun21toSep21)/2) %>% 
  select(c(Region, Mean_SeasDiff_rel_JuntoSep_20_21))

# Add column to correlation table
seasdiff_residence <- merge(seasdiff_residence, seasdiff_juntosep_20_21, by = "Region")

# Check if variable is normally distributed
shapiro.test(seasdiff_residence$Mean_SeasDiff_rel_JuntoSep_20_21)
```

```{r}
# Show table with different Estimates of Measurement
seasdiff_residence %>% 
  select(c(Region, Ratio_Port_Tourists_Aug19, Ratio_Port_Tourists_Summer_18_19, Ratio_Port_Tourists_Total_18_19, Mean_SeasDiff_rel_Apr20toDec20, Mean_SeasDiff_rel_Mar20toFeb21, Mean_SeasDiff_rel_JuntoSep_20_21)) %>% 
  rename("August 19" = Ratio_Port_Tourists_Aug19, "Apr 20 - Dec 20" = Mean_SeasDiff_rel_Apr20toDec20,"Jan 18 - Dec 19" = Ratio_Port_Tourists_Total_18_19, "Mar 20 - Feb 21" = Mean_SeasDiff_rel_Mar20toFeb21, "Summer 18 & 19" = Ratio_Port_Tourists_Summer_18_19, "Summer 20 & 21" = Mean_SeasDiff_rel_JuntoSep_20_21) %>% 
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Different Measures for Ratio of Portuguese Tourists & Decline in Tourism due to Covid-19</span>", digits = 4, format.args = list(big.mark = ","), align = "lcccccc") %>%
  column_spec(2:7, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), full_width = F) %>% 
  add_header_above(c(" " = 1, "Ratio of Portuguese Tourists" = 3, "Mean year-over-year rel. Change in Overnight Stays" = 3))
```

In order to quantify the weight of domestic tourism and the decline of tourism during the Covid-19 period in each region, two further measurements are employed for each variable. The p-value of each Shapiro-Wilk normality test is greater than 0.05, meaning that we do not reject the null hypothesis that the data follows a normal distribution. Let's see if there is a correlation between the other 8 combinations of variables:

```{r}
# Check linear relationship and if data has no outliers by looking at scatter plot
seasdiff_residence %>% 
  ggplot() +
  aes(x = Ratio_Port_Tourists_Aug19, y = Mean_SeasDiff_rel_Mar20toFeb21) +
  geom_point(colour = "#0c4c8a") +
  geom_smooth(formula = y ~ x, method = lm, se = FALSE) + # fits a linear model and adds a line to plot
  ggtitle("Scatter Plot", subtitle = "Ratio of Portuguese Tourists & Decline in Tourism due to Covid-19") +
  xlab("Ratio of Overnight Stays by Portuguese Tourists in Aug 2019") +
  ylab("Mean year-over-year rel. Change in Overnight Stays Mar 20 - Feb 21") 

# Pearson Correlation Test
cor.test(seasdiff_residence$Ratio_Port_Tourists_Aug19, seasdiff_residence$Mean_SeasDiff_rel_Mar20toFeb21)
```
...
seasdiff_residence %>% 
  ggplot() +
  aes(x = Ratio_Port_Tourists_Aug19, y = Mean_SeasDiff_rel_JuntoSep_20_21) +
  geom_point(colour = "#0c4c8a") +
  geom_smooth(formula = y ~ x, method = lm, se = FALSE) + # fits a linear model and adds a line to plot
  ggtitle("Scatter Plot", subtitle = "Ratio of Portuguese Tourists & Decline in Tourism due to Covid-19") +
  xlab("Ratio of Overnight Stays by Portuguese Tourists in Aug 2019") +
  ylab("Mean rel. Diff of Overnight Stays in Summer 20&21 to pre-Covid-19")

# Pearson Correlation Test
cor.test(seasdiff_residence$Ratio_Port_Tourists_Aug19, seasdiff_residence$Mean_SeasDiff_rel_JuntoSep_20_21)
```
...
seasdiff_residence %>% 
  ggplot() +
  aes(x = Ratio_Port_Tourists_Total_18_19, y = Mean_SeasDiff_rel_Apr20toDec20) +
  geom_point(colour = "#0c4c8a") +
  geom_smooth(formula = y ~ x, method = lm, se = FALSE) + # fits a linear model and adds a line to plot
  ggtitle("Scatter Plot", subtitle = "Ratio of Portuguese Tourists & Decline in Tourism due to Covid-19") +
  xlab("Ratio of Overnight Stays by Portuguese Tourists in 18&19") +
  ylab("Mean year-over-year rel. Change in Overnight Stays Apr-Dec 2020")

# Pearson Correlation Test
cor.test(seasdiff_residence$Ratio_Port_Tourists_Total_18_19, seasdiff_residence$Mean_SeasDiff_rel_Apr20toDec20)
```
...
seasdiff_residence %>% 
  ggplot() +
  aes(x = Ratio_Port_Tourists_Total_18_19, y = Mean_SeasDiff_rel_Mar20toFeb21) +
  geom_point(colour = "#0c4c8a") +
  geom_smooth(formula = y ~ x, method = lm, se = FALSE) + # fits a linear model and adds a line to plot
  ggtitle("Scatter Plot", subtitle = "Ratio of Portuguese Tourists & Decline in Tourism due to Covid-19") +
  xlab("Ratio of Overnight Stays by Portuguese Tourists in 18&19") +
...
# Pearson Correlation Test
cor.test(seasdiff_residence$Ratio_Port_Tourists_Total_18_19, seasdiff_residence$Mean_SeasDiff_rel_Mar20toFeb21)
```
...
seasdiff_residence %>% 
  ggplot() +
  aes(x = Ratio_Port_Tourists_Total_18_19, y = Mean_SeasDiff_rel_JuntoSep_20_21) +
  geom_point(colour = "#0c4c8a") +
  geom_smooth(formula = y ~ x, method = lm, se = FALSE) + # fits a linear model and adds a line to plot
  ggtitle("Scatter Plot", subtitle = "Ratio of Portuguese Tourists & Decline in Tourism due to Covid-19") +
  xlab("Ratio of Overnight Stays by Portuguese Tourists in 18&19") +
...
# Pearson Correlation Test
cor.test(seasdiff_residence$Ratio_Port_Tourists_Total_18_19, seasdiff_residence$Mean_SeasDiff_rel_JuntoSep_20_21)
```
...
seasdiff_residence %>% 
  ggplot() +
  aes(x = Ratio_Port_Tourists_Summer_18_19, y = Mean_SeasDiff_rel_Apr20toDec20) +
  geom_point(colour = "#0c4c8a") +
  geom_smooth(formula = y ~ x, method = lm, se = FALSE) + # fits a linear model and adds a line to plot
  ggtitle("Scatter Plot", subtitle = "Ratio of Portuguese Tourists & Decline in Tourism due to Covid-19") +
  xlab("Ratio of Overnight Stays by Portuguese Tourists in Summer 18&19") +
  ylab("Mean year-over-year rel. Change in Overnight Stays Apr-Dec 2020")

# Pearson Correlation Test
cor.test(seasdiff_residence$Ratio_Port_Tourists_Summer_18_19, seasdiff_residence$Mean_SeasDiff_rel_Apr20toDec20)
```
...
seasdiff_residence %>% 
  ggplot() +
  aes(x = Ratio_Port_Tourists_Summer_18_19, y = Mean_SeasDiff_rel_Mar20toFeb21) +
  geom_point(colour = "#0c4c8a") +
  geom_smooth(formula = y ~ x, method = lm, se = FALSE) + # fits a linear model and adds a line to plot
  ggtitle("Scatter Plot", subtitle = "Ratio of Portuguese Tourists & Decline in Tourism due to Covid-19") +
  xlab("Ratio of Overnight Stays by Portuguese Tourists in Summer 18&19") +
...
# Pearson Correlation Test
cor.test(seasdiff_residence$Ratio_Port_Tourists_Summer_18_19, seasdiff_residence$Mean_SeasDiff_rel_Mar20toFeb21)
```
...
seasdiff_residence %>% 
  ggplot() +
  aes(x = Ratio_Port_Tourists_Summer_18_19, y = Mean_SeasDiff_rel_JuntoSep_20_21) +
  geom_point(colour = "#0c4c8a") +
  geom_smooth(formula = y ~ x, method = lm, se = FALSE) + # fits a linear model and adds a line to plot
  ggtitle("Scatter Plot", subtitle = "Ratio of Portuguese Tourists & Decline in Tourism due to Covid-19") +
  xlab("Ratio of Overnight Stays by Portuguese Tourists in Summer 18&19") +
...
# Pearson Correlation Test
cor.test(seasdiff_residence$Ratio_Port_Tourists_Summer_18_19, seasdiff_residence$Mean_SeasDiff_rel_JuntoSep_20_21)
```

Three different measures were introduced for the ratio of Portuguese tourists in a given region and the decline in tourism due to Covid-19 in that region. There are 9 possible combinations between the variables. For each combination, a scatter plot is shown above and the Pearson's correlation test is carried out. The scatter plots show that there is always a linear relationship between the two variables and that there are no extreme outliers. 
Furthermore, the p-value of the Pearson's correlation test is always less than 0.05. This means that at the 5% significance level, we do reject the null hypothesis of no correlation. In other words, we conclude that there is always a linear relationship between the two variables in the population at the 5% significance level.
Hence, there is evidence that regions with a higher percentage of Portuguese tourists displayed lower declines in tourism demand during the Covid-19 period.
...
bind_rows(diff_2000, diff_2019) %>%
  ...
  column_spec(2:3, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = F) %>%
  footnote("Diff_to_August = Diff compared to Aug of the previous year")
...
#### RMSE Development with different Estimation Windows

##### Estimation Window from 2015

```{r, warning = FALSE}
# Create tsibble with tourism from 2015
tourism_madeira_from2015 <- tourism_madeira_until2022 %>% filter(year(Month) >= 2015)

# Compute RMSE and RMSSE for 2015 and create tibble
tourism_until_i <- tourism_madeira_from2015 %>%
    filter(year(Month) <= 2015)

accuracy_madeira_stretch_from2015 <- accuracy(fc_madeira_stretch, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2015)

# Create tsibble of summarized RMSE and RMSSE, beginning in 2016 and including more years
for (i in 2016:2022){
  tourism_until_i <- tourism_madeira_from2015 %>%
    filter(year(Month) <= i)
  ...
  accuracy_madeira_stretch_from2015 <- rbind(accuracy_madeira_stretch_from2015, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_madeira_stretch_from2015 <- accuracy_madeira_stretch_from2015 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_madeira_stretch_from2015 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
```{r}
# Plot development of RMSE for period 2015-2022
accuracy_madeira_stretch_from2015 %>%
  autoplot(RMSE) +
  ggtitle("RMSE Development by including more Years in Time Series", subtitle = "2015 - 2022") +
  guides(color = guide_legend(title = "Model", ncol = 2)) +
  xlab("Until Year") +
  ylab("RMSE")

# Calculate RMSE for 2015-2019 and according Rank
accuracy_2019 <- accuracy_madeira_stretch_from2015 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2015-2022 and according Rank
accuracy_2022 <- accuracy_madeira_stretch_from2015 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
  rename(RMSE_2022 = RMSE) %>%
  select(c(Rank_2022, Model, RMSE_2022))
...
# Show table
summary %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Change of RMSE: 2015-2019 vs. 2015-2022</span>", format.args = list(big.mark = ","), align = "lcccc") %>%
  column_spec(c(2, 4), border_left = T) %>%
  column_spec(c(2, 4), border_right = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), full_width = F) %>%
  add_header_above(c(" " = 1, "2015-2019" = 2, "2015-2022" = 2))
...
##### Estimation Window from 2017

```{r, warning = FALSE}
# Create tsibble with tourism from 2017
tourism_madeira_from2017 <- tourism_madeira_until2022 %>% filter(year(Month) >= 2017)

# Compute RMSE and RMSSE for 2017 and create tibble
tourism_until_i <- tourism_madeira_from2017 %>%
    filter(year(Month) <= 2017)

accuracy_madeira_stretch_from2017 <- accuracy(fc_madeira_stretch, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2017)

# Create tsibble of summarized RMSE and RMSSE, beginning in 2017 and including more years
for (i in 2018:2022){
  tourism_until_i <- tourism_madeira_from2017 %>%
    filter(year(Month) <= i)
  ...
  accuracy_madeira_stretch_from2017 <- rbind(accuracy_madeira_stretch_from2017, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_madeira_stretch_from2017 <- accuracy_madeira_stretch_from2017 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_madeira_stretch_from2017 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
```{r}
# Plot development of RMSE for period 2017-2022
accuracy_madeira_stretch_from2017 %>%
  autoplot(RMSE) +
  ggtitle("RMSE Development by including more Years in Time Series", subtitle = "2017 - 2022") +
  guides(color = guide_legend(title = "Model", ncol = 2)) +
  xlab("Until Year") +
  ylab("RMSE")

# Calculate RMSE for 2017-2019 and according Rank
accuracy_2019 <- accuracy_madeira_stretch_from2017 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2017-2022 and according Rank
accuracy_2022 <- accuracy_madeira_stretch_from2017 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
# Show table
summary %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Change of RMSE: 2017-2019 vs. 2017-2022</span>", format.args = list(big.mark = ","), align = "lcccc") %>%
  column_spec(c(2, 4), border_left = T) %>%
  column_spec(c(2, 4), border_right = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), full_width = F) %>%
  add_header_above(c(" " = 1, "2017-2019" = 2, "2017-2022" = 2))
...
##### Estimation Window from 2019

```{r, warning = FALSE}
# Create tsibble with tourism from 2019
tourism_madeira_from2019 <- tourism_madeira_until2022 %>% filter(year(Month) >= 2019)

# Compute RMSE and RMSSE for 2019 and create tibble
tourism_until_i <- tourism_madeira_from2019 %>%
    filter(year(Month) <= 2019)

accuracy_madeira_stretch_from2019 <- accuracy(fc_madeira_stretch, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2019)

# Create tsibble of summarized RMSE and RMSSE, beginning in 2020 and including more years
for (i in 2020:2022){
  tourism_until_i <- tourism_madeira_from2019 %>%
    filter(year(Month) <= i)
  ...
  accuracy_madeira_stretch_from2019 <- rbind(accuracy_madeira_stretch_from2019, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_madeira_stretch_from2019 <- accuracy_madeira_stretch_from2019 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_madeira_stretch_from2019 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
```{r}
# Plot development of RMSE for period 2019-2022
accuracy_madeira_stretch_from2019 %>%
  autoplot(RMSE) +
  ggtitle("RMSE Development by including more Years in Time Series", subtitle = "2019 - 2022") +
  guides(color = guide_legend(title = "Model", ncol = 2)) +
  xlab("Until Year") +
  ylab("RMSE")

# Calculate RMSE for 2019-2019 and according Rank
accuracy_2019 <- accuracy_madeira_stretch_from2019 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2019-2022 and according Rank
accuracy_2022 <- accuracy_madeira_stretch_from2019 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
# Show table
summary %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Change of RMSE: 2019-2019 vs. 2019-2022</span>", format.args = list(big.mark = ","), align = "lcccc") %>%
  column_spec(c(2, 4), border_left = T) %>%
  column_spec(c(2, 4), border_right = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), full_width = F) %>%
  add_header_above(c(" " = 1, "2019-2019" = 2, "2019-2022" = 2))
...
accuracy_madeira_slide_from2015 <- accuracy(fc_madeira_slide, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2015)
...
for (i in 2016:2022){
  ...
  accuracy_madeira_slide_from2015 <- rbind(accuracy_madeira_slide_from2015, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_madeira_slide_from2015 <- accuracy_madeira_slide_from2015 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_madeira_slide_from2015 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
# Calculate RMSE for 2015-2019 and according Rank
accuracy_2019 <- accuracy_madeira_slide_from2015 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2015-2022 and according Rank
accuracy_2022 <- accuracy_madeira_slide_from2015 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
accuracy_madeira_slide_from2017 <- accuracy(fc_madeira_slide, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2017)
...
for (i in 2018:2022){
  ...
  accuracy_madeira_slide_from2017 <- rbind(accuracy_madeira_slide_from2017, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_madeira_slide_from2017 <- accuracy_madeira_slide_from2017 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_madeira_slide_from2017 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
# Calculate RMSE for 2017-2019 and according Rank
accuracy_2019 <- accuracy_madeira_slide_from2017 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2017-2022 and according Rank
accuracy_2022 <- accuracy_madeira_slide_from2017 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
accuracy_madeira_slide_from2019 <- accuracy(fc_madeira_slide, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2019)
...
for (i in 2020:2022){
  ...
  accuracy_madeira_slide_from2019 <- rbind(accuracy_madeira_slide_from2019, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_madeira_slide_from2019 <- accuracy_madeira_slide_from2019 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_madeira_slide_from2019 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
# Calculate RMSE for 2019-2019 and according Rank
accuracy_2019 <- accuracy_madeira_slide_from2019 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2019-2022 and according Rank
accuracy_2022 <- accuracy_madeira_slide_from2019 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
```{r, warning = FALSE}
# Create tsibble with tourism from 2015
tourism_algarve_from2015 <- tourism_algarve_until2022 %>% filter(year(Month) >= 2015)

# Compute RMSE and RMSSE for 2015 and create tibble
tourism_until_i <- tourism_algarve_from2015 %>%
    filter(year(Month) <= 2015)

accuracy_algarve_stretch_from2015 <- accuracy(fc_algarve_stretch, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2015)
...
for (i in 2016:2022){
  ...
  accuracy_algarve_stretch_from2015 <- rbind(accuracy_algarve_stretch_from2015, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_algarve_stretch_from2015 <- accuracy_algarve_stretch_from2015 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_algarve_stretch_from2015 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
# Calculate RMSE for 2015-2019 and according Rank
accuracy_2019 <- accuracy_algarve_stretch_from2015 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2015-2022 and according Rank
accuracy_2022 <- accuracy_algarve_stretch_from2015 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
```{r, warning = FALSE}
# Create tsibble with tourism from 2017
tourism_algarve_from2017 <- tourism_algarve_until2022 %>% filter(year(Month) >= 2017)

# Compute RMSE and RMSSE for 2017 and create tibble
tourism_until_i <- tourism_algarve_from2017 %>%
    filter(year(Month) <= 2017)

accuracy_algarve_stretch_from2017 <- accuracy(fc_algarve_stretch, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2017)
...
for (i in 2018:2022){
  ...
  accuracy_algarve_stretch_from2017 <- rbind(accuracy_algarve_stretch_from2017, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_algarve_stretch_from2017 <- accuracy_algarve_stretch_from2017 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_algarve_stretch_from2017 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
# Calculate RMSE for 2017-2019 and according Rank
accuracy_2019 <- accuracy_algarve_stretch_from2017 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2017-2022 and according Rank
accuracy_2022 <- accuracy_algarve_stretch_from2017 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
```{r, warning = FALSE}
# Create tsibble with tourism from 2019
tourism_algarve_from2019 <- tourism_algarve_until2022 %>% filter(year(Month) >= 2019)

# Compute RMSE and RMSSE for 2019 and create tibble
tourism_until_i <- tourism_algarve_from2019 %>%
    filter(year(Month) <= 2019)

accuracy_algarve_stretch_from2019 <- accuracy(fc_algarve_stretch, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2019)
...
for (i in 2020:2022){
  ...
  accuracy_algarve_stretch_from2019 <- rbind(accuracy_algarve_stretch_from2019, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_algarve_stretch_from2019 <- accuracy_algarve_stretch_from2019 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_algarve_stretch_from2019 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
# Calculate RMSE for 2019-2019 and according Rank
accuracy_2019 <- accuracy_algarve_stretch_from2019 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2019-2022 and according Rank
accuracy_2022 <- accuracy_algarve_stretch_from2019 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
accuracy_algarve_slide_from2015 <- accuracy(fc_algarve_slide, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2015)
...
for (i in 2016:2022){
  ...
  accuracy_algarve_slide_from2015 <- rbind(accuracy_algarve_slide_from2015, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_algarve_slide_from2015 <- accuracy_algarve_slide_from2015 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_algarve_slide_from2015 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
# Calculate RMSE for 2015-2019 and according Rank
accuracy_2019 <- accuracy_algarve_slide_from2015 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2015-2022 and according Rank
accuracy_2022 <- accuracy_algarve_slide_from2015 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
accuracy_algarve_slide_from2017 <- accuracy(fc_algarve_slide, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2017)
...
for (i in 2018:2022){
  ...
  accuracy_algarve_slide_from2017 <- rbind(accuracy_algarve_slide_from2017, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_algarve_slide_from2017 <- accuracy_algarve_slide_from2017 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_algarve_slide_from2017 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
# Calculate RMSE for 2017-2019 and according Rank
accuracy_2019 <- accuracy_algarve_slide_from2017 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2017-2022 and according Rank
accuracy_2022 <- accuracy_algarve_slide_from2017 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
accuracy_algarve_slide_from2019 <- accuracy(fc_algarve_slide, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2019)
...
for (i in 2020:2022){
  ...
  accuracy_algarve_slide_from2019 <- rbind(accuracy_algarve_slide_from2019, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_algarve_slide_from2019 <- accuracy_algarve_slide_from2019 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_algarve_slide_from2019 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
# Calculate RMSE for 2019-2019 and according Rank
accuracy_2019 <- accuracy_algarve_slide_from2019 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2019-2022 and according Rank
accuracy_2022 <- accuracy_algarve_slide_from2019 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
```{r, warning = FALSE}
# Create tsibble with tourism from 2015
tourism_alentejo_from2015 <- tourism_alentejo_until2022 %>% filter(year(Month) >= 2015)

# Compute RMSE and RMSSE for 2015 and create tibble
tourism_until_i <- tourism_alentejo_from2015 %>%
    filter(year(Month) <= 2015)

accuracy_alentejo_stretch_from2015 <- accuracy(fc_alentejo_stretch, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2015)
...
for (i in 2016:2022){
  ...
  accuracy_alentejo_stretch_from2015 <- rbind(accuracy_alentejo_stretch_from2015, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_alentejo_stretch_from2015 <- accuracy_alentejo_stretch_from2015 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_alentejo_stretch_from2015 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
# Calculate RMSE for 2015-2019 and according Rank
accuracy_2019 <- accuracy_alentejo_stretch_from2015 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2015-2022 and according Rank
accuracy_2022 <- accuracy_alentejo_stretch_from2015 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
```{r, warning = FALSE}
# Create tsibble with tourism from 2017
tourism_alentejo_from2017 <- tourism_alentejo_until2022 %>% filter(year(Month) >= 2017)

# Compute RMSE and RMSSE for 2017 and create tibble
tourism_until_i <- tourism_alentejo_from2017 %>%
    filter(year(Month) <= 2017)

accuracy_alentejo_stretch_from2017 <- accuracy(fc_alentejo_stretch, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2017)
...
for (i in 2018:2022){
  ...
  accuracy_alentejo_stretch_from2017 <- rbind(accuracy_alentejo_stretch_from2017, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_alentejo_stretch_from2017 <- accuracy_alentejo_stretch_from2017 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_alentejo_stretch_from2017 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
# Calculate RMSE for 2017-2019 and according Rank
accuracy_2019 <- accuracy_alentejo_stretch_from2017 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2017-2022 and according Rank
accuracy_2022 <- accuracy_alentejo_stretch_from2017 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
```{r, warning = FALSE}
# Create tsibble with tourism from 2019
tourism_alentejo_from2019 <- tourism_alentejo_until2022 %>% filter(year(Month) >= 2019)

# Compute RMSE and RMSSE for 2019 and create tibble
tourism_until_i <- tourism_alentejo_from2019 %>%
    filter(year(Month) <= 2019)

accuracy_alentejo_stretch_from2019 <- accuracy(fc_alentejo_stretch, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2019)
...
for (i in 2020:2022){
  ...
  accuracy_alentejo_stretch_from2019 <- rbind(accuracy_alentejo_stretch_from2019, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_alentejo_stretch_from2019 <- accuracy_alentejo_stretch_from2019 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_alentejo_stretch_from2019 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
# Calculate RMSE for 2019-2019 and according Rank
accuracy_2019 <- accuracy_alentejo_stretch_from2019 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2019-2022 and according Rank
accuracy_2022 <- accuracy_alentejo_stretch_from2019 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
accuracy_alentejo_slide_from2015 <- accuracy(fc_alentejo_slide, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2015)
...
for (i in 2016:2022){
  ...
  accuracy_alentejo_slide_from2015 <- rbind(accuracy_alentejo_slide_from2015, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_alentejo_slide_from2015 <- accuracy_alentejo_slide_from2015 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_alentejo_slide_from2015 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
# Calculate RMSE for 2015-2019 and according Rank
accuracy_2019 <- accuracy_alentejo_slide_from2015 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2015-2022 and according Rank
accuracy_2022 <- accuracy_alentejo_slide_from2015 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
accuracy_alentejo_slide_from2017 <- accuracy(fc_alentejo_slide, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2017)
...
for (i in 2018:2022){
  ...
  accuracy_alentejo_slide_from2017 <- rbind(accuracy_alentejo_slide_from2017, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_alentejo_slide_from2017 <- accuracy_alentejo_slide_from2017 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_alentejo_slide_from2017 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
# Calculate RMSE for 2017-2019 and according Rank
accuracy_2019 <- accuracy_alentejo_slide_from2017 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2017-2022 and according Rank
accuracy_2022 <- accuracy_alentejo_slide_from2017 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
accuracy_alentejo_slide_from2019 <- accuracy(fc_alentejo_slide, tourism_until_i) %>%
  select(c(.model, RMSE, RMSSE)) %>%
  mutate(Until_Year = 2019)
...
for (i in 2020:2022){
  ...
  accuracy_alentejo_slide_from2019 <- rbind(accuracy_alentejo_slide_from2019, accuracy_next_year)
}

rm(accuracy_next_year, tourism_until_i) # remove variables from R environment

accuracy_alentejo_slide_from2019 <- accuracy_alentejo_slide_from2019 %>%
  rename(Model = .model) %>%
  select(c(Until_Year, Model, RMSE, RMSSE)) %>%
  as_tsibble(key = Model, index = Until_Year)

# Show table
accuracy_alentejo_slide_from2019 %>%
  mutate(across(RMSE, ~ format(.x, big.mark = ","))) %>%
  kbl(caption = "<span style='color:black;font-size:medium;font-weight:bold'>Development of RMSE and RMSSE by including more Years in Time Series</span>", digits = 2, align = "cccc") %>%
  column_spec(2:4, border_left = "2px solid lightgray") %>%
  kable_styling(bootstrap_options = c("striped", "hover","condensed"), fixed_thead = T, full_width = F) %>%
...
# Calculate RMSE for 2019-2019 and according Rank
accuracy_2019 <- accuracy_alentejo_slide_from2019 %>% 
  filter(Until_Year == 2019) %>% 
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2019 = row_number()) %>%
...
# Calculate RMSE for 2019-2022 and according Rank
accuracy_2022 <- accuracy_alentejo_slide_from2019 %>% 
  filter(Until_Year == 2022) %>%
  as_tibble() %>% 
  arrange(RMSE) %>%
  mutate(Rank_2022 = row_number()) %>%
...
# Improving Forecasting Accuracy following Data Anomalies
...
rownames(rmse_adjusted_outliers_summary_total) <- c("Original_Outliers", "Adj_Outliers_Mean_20yrs", "Adj_Outliers_Mean_10_yrs", "Adj_Outliers_Mean_5yrs", "Adj_Outliers_50%_Mean_20yrs", "Adj_Outliers_50%_Mean_10yrs", "Adj_Outliers_50%_Mean_5yrs")
